...
Los nueve problemas siguientes no pretenden agotar el suplemento. Funcionan como semilla
...
\]
donde \(L\) es el nivel en decibelios e \(I_0\) es la intensidad de referencia.
...
\begin{enumerate}
  ...
  \item fijar el mayor nivel admisible en una tercera zona para que su intensidad no supere en
  mil veces a la de la sala de estudio.
\end{enumerate}
\end{contextproblem}
...
El margen semanal \(M(n)\), en centenas de euros, de un taller que fabrica lotes numerados se
modeliza por
\[
M(n)=n^3-4n^2-n+4.
\]
Se pide:
\begin{enumerate}
  \item factorizar el polinomio para detectar semanas de equilibrio;
...
M(n)=(n-2)(n^2-2n-5)-6.
\]
Los puntos de equilibrio son \(n=1\), \(n=-1\) y \(n=4\). En \(n=2\) el margen no se anula:
\(M(2)=-6\).
\end{shortanswer}
...
\]
Luego los ceros del modelo son
\[
...
M(n)=(n-2)(n^2-2n-5)-6.
\]
El resto es \(-6\), que coincide con \(M(2)\) por el teorema del resto. Por tanto, al tomar
\(n=2\) el margen vale \(-6\) centenas de euros y no se alcanza equilibrio.
\end{fullsolution}
...
\]
Con las restricciones \(x\geq 20\) e \(y\geq 10\), los vertices de la region son:
\[
...
\]
El mayor valor es \(2025\), asi que el plan optimo es producir \(50\) cuadernos y \(45\)
archivadores.
\end{fullsolution}

\section{C05. Observacion de una torre desde dos puntos}

\begin{contextproblem}{Topografia de una ladera}
Desde un sendero recto se observa la parte superior de una torre. En el punto \(A\) el angulo
...
La altura es aproximadamente \(53.4\) metros.
\end{shortanswer}
...
\]
Por tanto, la torre mide aproximadamente \(\SI{53.4}{m}\).
\end{fullsolution}
...
\]
Como el coseno con \(\vec a\) es mucho mayor, la ruta final queda claramente mas alineada con
\(\vec a\).
\end{fullsolution}
...
\begin{enumerate}
  ...
  \item calcular a partir de que instante la temperatura baja de \(5^\circ\).
\end{enumerate}
\end{contextproblem}
...
t>3.
\]
Por tanto, la temperatura cae por debajo de \(5^\circ\) despues de tres horas.
\end{fullsolution}
...
A''(x)=-4<0,
\]
luego el punto critico corresponde a un maximo. Sustituyendo,
\[
y=20-2\cdot 5=10.
\]
El area maxima vale
...
A(5)=5\cdot 10=50.
\]
Las dimensiones optimas son \(5\) metros de profundidad y \(10\) metros de ancho.
\end{fullsolution}
...
Cuando este proyecto se complete como cuaderno final, el PDF deberia quedar organizado en nueve
